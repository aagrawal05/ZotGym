...
export default function SignupForm() {
  ...
  const availableSports = ["weightlifting", "basketball", "running", "badminton", "soccer", "volleyball"];
  ...
  const handleSubmit = async (e) => {
    ...
    schedule.forEach((day, i) => 
        day.forEach((time, j) =>
            {
                if (time)
                    computed_times += days[i]+" "+times[j]+", "
            }
        )
    )
    console.log(computed_times)
    const userData = await fetch("http://127.0.0.1:5000/signup", {
      method: "POST",
      headers: {
          "Content-Type": "application/json"
      },
      body: JSON.stringify({
          workout_time: computed_times,
          physical_interest: selectedSports.join(', '),
          ...formData
      })
    }).then(res => res.json())
    if (userData) {
